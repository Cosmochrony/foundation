...
the moment of resolution.
  ...
  \subsection{Entanglement as shared ancestral fibre}
    ...
    This qualitative picture is now quantitatively located within the spectral
    admissibility cascade.
    For a closed conjugate Weil pair $\{c,q-c\}$, the minimal admissible projection
    is spectrally atomic: after removal of the zero mode it is the two-dimensional
    Born--Infeld parity pair, identified with the spin-$\tfrac12$ representation, and
    its reduced state is the singlet with entanglement entropy
    $\log 2$ — the entropy of the state behind the singlet correlator and the
    Tsirelson bound~\cite{Beau2026ent2,Beau2026q1}.
    A finer pipeline-level observable, the full residual fibre-level rank
    $r_{\mathrm{pair}}(n)$, gives $S_{\mathrm{ent}}(n)=\log r_{\mathrm{pair}}(n)$,
    proved for matched single-character blocks and conditional otherwise on a residual
    indiscernibility hypothesis~\cite{Beau2026entnote,Beau2026ent1}.
    The two observables must not be conflated; they coincide only when
    $r_{\mathrm{pair}}(n)=2$.
