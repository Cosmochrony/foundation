...
on effective field configurations~\cite{BornInfeld}.
  ...
  \subsection{Non-factorisability and the emergence of Heisenberg structure}
    ...
    \begin{proof}
      Let $X$ and $\sigma(X)$ denote the two generators of a conjugate pair in
      $F_n$, related by the Born--Infeld parity involution $\sigma\colon c \mapsto q-c$.

      \smallskip
      \noindent\textit{Step~1: non-trivial commutator.}
      Suppose $[X, \sigma(X)] = 0$.
      Then $F_n$ decomposes as $\bigoplus_\lambda F_\lambda$, the simultaneous
      eigenspace decomposition stable under $X$ and $\sigma(X)$.
      By Lemma~\ref{lem:non-factorisation}, this decomposition is forbidden by A3.
      Therefore $[X, \sigma(X)] \neq 0$.

      \smallskip
      \noindent\textit{Step~2: central commutator.}
      Set $Z := [X, \sigma(X)] \neq 0$.
      Suppose $Z$ is not central: there exists a generator $Y$ with $[Z, Y] \neq 0$.
      The algebra generated by $(X, \sigma(X), Z)$ then strictly contains the
      algebra generated by the conjugate pair $(X, \sigma(X))$ alone.
      By A1, $F_n$ is generated minimally by the conjugate pair — every internal
      structure is determined by the admissibility constraints carried by that pair,
      and no independent generator may extend the algebra.
      A strictly richer generative closure contradicts this minimality.
      Therefore $Z$ is central.

      \smallskip
      \noindent\textit{Step~3: Heisenberg algebra.}
      The group $\langle X, \sigma(X) \rangle$ is a non-trivial central extension
      generated by a conjugate pair, with no independent relation beyond those
      forced by minimality (A1).
      By the classification of central extensions of $(\mathbb{Z}/q\mathbb{Z})^2$
      by $\mathbb{Z}/q\mathbb{Z}$ (group cohomology), every such extension with
      minimal generators and without additional independent relations is isomorphic
      to the Heisenberg algebra $\mathrm{Heis}_3(\mathbb{Z}/q\mathbb{Z})$.
      The commutation relation takes the form
      \begin{equation}
        [X,\,\sigma(X)] \;=\; Z, \qquad Z \text{ central}, \qquad Z \neq 0,
      \end{equation}
      which defines a discrete symplectic structure on $\langle X, \sigma(X)\rangle$.

      \smallskip
      \noindent\textit{Step~4: prime order.}
      Suppose $|Z| = pq$ with $p, q > 1$.
      The centre $\langle Z\rangle$ then admits proper non-trivial subgroups, and
      $F_n$ decomposes into sub-representations indexed by the characters of
      $\langle Z\rangle$:
      \[
        F_n \;=\; \bigoplus_\chi F_\chi.
      \]
      This decomposition is stable under all generators (since the centre commutes
      with everything), hence stable under admissible generators.
      By Lemma~\ref{lem:non-factorisation}, it is forbidden by A3.
      Therefore $|Z| = q$ is prime.

      \smallskip
      \noindent\textit{Step~5: identification $F_n \simeq V_\rho$.}
      The symmetry group of $F_n$ is isomorphic to
      $\mathrm{Heis}_3(\mathbb{Z}/q\mathbb{Z})$.
      By the discrete Stone--von Neumann theorem, the unique irreducible
      representation of $\mathrm{Heis}_3(\mathbb{Z}/q\mathbb{Z})$ with
      non-trivial central character is the Weil representation $V_\rho$ on
      $L^2(\mathbb{Z}/q\mathbb{Z})$.
      Born--Infeld parity forces $c \neq 0$~\cite{BornInfeld}, so the central
      character is non-trivial.
      Therefore $F_n \simeq V_\rho$.
    \end{proof}

    \begin{remark}[Derivation chain]
      The logical chain of Theorem~\ref{thm:heisenberg} may be summarised as:
      \begin{equation}
        \begin{aligned}
          \text{A3}
...
the moment of resolution.
  ...
  \subsection*{Open directions}
    ...
    \paragraph{Q5 — Proto-state ontology.}
      ...
      Theorem~\ref{thm:heisenberg} closes the primary open question of this
      direction: the space in which the proto-state exists between transitions
      is $F_n \simeq V_\rho$, the Weil representation of
      $\mathrm{Heis}_3(\mathbb{Z}/q\mathbb{Z})$, determined uniquely by A1--A3
      and Born--Infeld parity.
      The relationship between this discrete structure and continuous spacetime
      geometry in the large-$q$ limit (Section~\ref{subsec:continuum-limit})
      remains the subject of the companion spectral geometry programme.

    \paragraph{Q6 — Uncertainty principle as fibre incompatibility.}
      This direction is resolved by Theorem~\ref{thm:heisenberg} and
      Remark~\ref{rem:uncertainty}: the Heisenberg uncertainty principle follows
      as a corollary of $[X, \sigma(X)] \neq 0$, which is itself a consequence
      of A3 alone.
      The uncertainty principle is a structural property of the non-resolved
      proto-state, not an independent postulate about observables.
